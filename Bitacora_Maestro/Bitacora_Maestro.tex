...
\begin{document}
	...
	Este documento describe detalladamente el proceso de cifrado de imágenes implementado para el sistema maestro, el cual se ejecuta en una Raspberry Pi 4. El sistema utiliza técnicas criptográficas avanzadas basadas en \textbf{sistemas caóticos} para cifrar imágenes RGB mediante dos etapas principales:
	
	\begin{enumerate}
		\item \textbf{Difusión} (usando el Mapa Logístico)
		\item \textbf{Confusión} (usando el Oscilador de Rössler y el Mapa Logístico)
	\end{enumerate}
	...
	\subsection*{Imagen de referencia:}
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.5\textwidth]{Prueba2.jpg}
		\caption{Imagen de ejemplo utilizada para el proceso de cifrado}
	...
	El histograma para la imagen de referencia fue:
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.7\textwidth]{histograma_original.png}
		\caption{Histograma de la imagen de referencia}
		\label{fig:hist_original}
	\end{figure}
	
	\section{Inicialización y configuración de parámetros}
...
CA_CERT_PATH = "/etc/mosquitto/ca_certificates/ca.crt"
	...
	\section{Carga y vectorización de la imagen}
	...
	\subsection{Representación en vectores}
	...
	\section{Etapa de Difusión (Mapa Logístico)}
	...
	El algoritmo de difusión utiliza un \textbf{marcador (260.0)} para llevar un control de qué valores ya han sido utilizados. Se realizan dos pasadas:
...
for j in range(nmax):
	...
	\begin{table}[H]
		...
		\begin{tabular}{@{}ccccc@{}}
			...
			9 & 53309 & 0.654902 & 0.815686 & Usado \\ \bottomrule
		\end{tabular}
	\end{table}
	...
	\begin{figure}[H]
		...
		\caption{Resultado de la etapa de difusión (pseudo-imagen)}
		\label{fig:img_difusion}
	\end{figure}
	
	También se hace un análisis para obtener el histograma de la imagen después de la difusión.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.7\textwidth]{histograma_difusion.png}
		\caption{Histograma de la etapa de difusión}
		\label{fig:hist_difusion}
	\end{figure}
	...
	\begin{itemize}
		...
		\item Posterior al proceso, se aplica un factor de escala: $\text{difusion} = \text{difusion} \times \text{IMG\_SCALE} = \text{difusion} \times 0.01$
	\end{itemize}
	...
	\begin{table}[H]
		...
		\begin{tabular}{@{}ccc@{}}
			...
			9 & 0.815686 & 0.00815686 \\ \bottomrule
		\end{tabular}
	\end{table}
	
	\section{Etapa de confusión (Oscilador de Rössler)}
	...
	\textbf{Nota}: hice el cambio a RK23, debido a que me brindó un mejor resultado de sincronización en el esclavo, aunque con RK45 también se obtuvieron buenos resultados e igual se logró una distancia hamming de 0.
	
	\subsubsection*{Tolerancias:}
	\begin{itemize}
		\item Relativa: $\text{rtol} = 10^{-6}$
		\item Absoluta: $\text{atol} = 10^{-6}$
	\end{itemize}
	...
	\textbf{Nota}: Los valores mostrados son de ejemplo. Los valores reales dependen de la integración numérica.
	
	\subsection{Tiempo de Sincronización}
	
	Los primeros $\text{TIEMPO\_SINC} = 6000$ puntos se utilizan para:
	\begin{enumerate}
		\item \textbf{Permitir que el sistema alcance su atractor caótico al momento de sincronizar}
...
x_cif = np.resize(x_key, nmax)
	...
	\begin{table}[H]
		...
		\begin{tabular}{@{}ccccc@{}}
			...
			9 & 0.00815686 & 0.284316 & -8.297385 & -7.004912 \\ \bottomrule
		\end{tabular}
	\end{table}
	...
	\begin{figure}[H]
		...
		\caption{Resultado del proceso de Confusión}
		\label{fig:img_confusion}
	\end{figure}

	El histograma obtenido de la imagen encriptada fue:
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.7\textwidth]{histograma_cifrado.png}
		\caption{Histograma de la etapa de confusión}
		\label{fig:hist_confusion}
	\end{figure}
	...
	\section{Etapa de preparación de datos}
...
data = {
	...
	\subsection{Parámetros de Seguridad (Keys)}
	...
	\section{Etapa de transmisión mediante MQTT/TLS}
	...
	\begin{itemize}
		\item \textbf{Encriptación}: Los datos se cifran durante la transmisión, añadiendo otra capa de seguridad.
...
client.disconnect()
	...
	\begin{table}[H]
		...
		\begin{tabular}{@{}lcc@{}}
			...
			\textbf{Total del programa} & \textbf{9.9951} & \textbf{100\%} \\ \bottomrule
		\end{tabular}
	\end{table}
	
	\textit{Nota: Los tiempos son de ejemplo y varían según el hardware y la velocidad de la red.}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.5\textwidth]{diagrama_dispersion.png}
		\caption{Diagrama de dispersión, imagen encriptada}
		\label{fig:img_dispersion}
	\end{figure}
	...
	\begin{table}[H]
		...
		\begin{tabular}{@{}lc@{}}
			...
			Porcentaje de bits cambiados & 49.15\% \\ \bottomrule
		\end{tabular}
	\end{table}
